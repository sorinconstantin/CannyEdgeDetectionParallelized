...
void canny(unsigned char *image, int rows, int cols, float sigma,
         float tlow, float thigh, unsigned char **edge, float *tempim, char *fname);
void gaussian_smooth_y(unsigned char *image, int rows, int cols, float sigma, float *tempim, short int **smoothedim);
...
void *gaussian_smooth_x(void *arg)
{
   ...
   int blockSizeR, start, end, cols;
   blockSizeR = info->rows / info->nr_of_threads;

   /****************************************************************************
   * Create a 1-dimensional gaussian smoothing kernel.
   ****************************************************************************/
   if(VERBOSE) printf("   Computing the gaussian smoothing kernel.\n");
   make_gaussian_kernel(info->sigma, &kernel, &windowsize);
   center = windowsize / 2;
   
   /****************************************************************************
   * Blur in the x - direction.
   ****************************************************************************/
   start = info->thread_nr * blockSizeR;
   cols = info->cols;
   
   if (info->thread_nr == info->nr_of_threads - 1){
      end = info->rows;
   }
   else{
      end = start + blockSizeR;
   } 

   if(VERBOSE) printf("Thread: %d   Bluring the image in the X-direction.\n", info->thread_nr);
   for(r = start; r < end; r++){
      for(c=0;c<cols;c++){
         dot = 0.0;
         sum = 0.0;
...
int main(int argc, char *argv[])
{
   ...
   int rows, cols, N, i;           /* The dimensions of the image. */
   ...
   ****************************************************************************/
   if(argc < 6){
   fprintf(stderr,"\n<USAGE> %s image sigma tlow thigh <nr_of_threads>[writedirim]\n",argv[0]);
   ...
   N = atof(argv[5]);
   struct thread_info threads_info[N];
   ...
   ****************************************************************************/
   ...
   for (i = 0; i < N; i++){
      threads_info[i].rows = rows;
      threads_info[i].cols = cols;
      threads_info[i].thread_nr = i;
      threads_info[i].nr_of_threads = N;
      threads_info[i].sigma = sigma;
      int resC = pthread_create(&threads[i], NULL, gaussian_smooth_x, &(threads_info[i]));
      printf("created thread #%d; status %d\n", i, resC);
   }

   for (i = 0; i < N; i++){
      int resJ = pthread_join(threads[i], NULL);
      printf("thread #%d finished!\n", i);
   }

   canny(image, rows, cols, sigma, tlow, thigh, &edge, tempim, dirfilename);
   ...
   ****************************************************************************/
   ...
   gettimeofday(&end, 0);
   double pass = (end.tv_sec - start.tv_sec) + (double)(end.tv_usec - start.tv_usec)/1000000.0;
         ...
         float tlow, float thigh, unsigned char **edge, float *tempim, char *fname)
{
   ...
   * deviation.
   ****************************************************************************/
   if(VERBOSE) printf("Y-Smoothing the image using a gaussian kernel.\n");
   gaussian_smooth_y(image, rows, cols, sigma, tempim, &smoothedim);
...
* PURPOSE: Blur an image with a gaussian filter on y dir.
* NAME: Mike Heath
* DATE: 2/15/96
*******************************************************************************/
void gaussian_smooth_y(unsigned char *image, int rows, int cols, float sigma, float *tempim, short int **smoothedim)
